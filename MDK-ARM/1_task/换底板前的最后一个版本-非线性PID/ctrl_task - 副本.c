...
//		MPU_Get_Accelerometer(&imu.accel[0],&imu.accel[1],&imu.accel[2]);	//µ√µΩº”ÀŸ∂»¥´∏–∆˜ ˝æ›
//		MPU_Get_Gyroscope(&imu.gyro[0],&imu.gyro[1],&imu.gyro[2]);	//µ√µΩÕ”¬›“« ˝æ›
////		imu.temp=MPU_Get_Temperature();	              //µ√µΩŒ¬∂»÷µ
//		sys_variable.theta = imu.pitch;
////		taskEXIT_CRITICAL();
//		
//		/*balance circle | current circle*/
//		balance_pid_calc(imu.pitch, sys_variable.mid_angle, imu.gyro[1]);
//		
//		/*speed circle*/
//		speed_circle_cnt++;
...
//		{
//			speed_circle_cnt= 0 ;
//			sys_variable.speed_out = pid_calc(&pid_move_speed, sys_variable.v, sys_variable.set_v_speed);//◊Ó∫Û“ª∏ˆ≤Œ ˝ «ƒø±ÍÀŸ∂»£¨’‚¿Ô‘› ±≤ª”√£¨∏¯0		     
//		}
//		/*direction circle*/
//		sys_variable.turn_out = pid_calc(&pid_turn, 0, 0);		

//		sys_variable.totall_out[WHEEL_L] = (int16_t)balance_pid.balance_out[WHEEL_L] + sys_variable.speed_out + sys_variable.turn_out;
//		sys_variable.totall_out[WHEEL_R] = (int16_t)-balance_pid.balance_out[WHEEL_R] - sys_variable.speed_out + sys_variable.turn_out;
		sys_variable.totall_out[WHEEL_L] = test_current;
		sys_variable.totall_out[WHEEL_R] = test_current;
		sys_variable.totall_out[2] = test_current;
		sys_variable.totall_out[3] = test_current;
		/* Êï¥‰ΩìÊéßÂà∂ÁîµÊµÅÈôêÂπÖ */
		...
		VAL_LIMIT(sys_variable.totall_out[2], -MAX_WHEEL_RPM, MAX_WHEEL_RPM);
		VAL_LIMIT(sys_variable.totall_out[3], -MAX_WHEEL_RPM, MAX_WHEEL_RPM);
...
////		}
		...
		sprintf(lcd_buf, "speed0: %5d   speed1: %5d   speed2: %5d   speed3: %5d   \n", moto_chassis[0].speed_rpm, moto_chassis[1].speed_rpm, moto_chassis[2].speed_rpm, moto_chassis[3].speed_rpm);
...
float balance_pid_calc(float get_pitch, float set_pitch, int16_t gyro_y)
{
	...
	VAL_LIMIT(balance_pid.err, -10.0f, 10.0f);
	err_tan = 0.1047f*balance_pid.err;/* 0.1047Ê†πÊçÆ15¬∞ÁÆóÂá∫ */
	/* ËÆ°ÁÆóKPËæìÂá∫ */
	...
	/* ËæìÂá∫Ë°•ÂÅøÔºåÊ≠§È°πÊ†πÊçÆÁîµÊú∫ÁâπÊÄß‰∏çÂêå‰øÆÊîπ */
	balance_pid.balance_out[WHEEL_L] = balance_pid.out + balance_pid.cur_comp[WHEEL_L];
	balance_pid.balance_out[WHEEL_R] = balance_pid.out + balance_pid.cur_comp[WHEEL_R];
	/* Âπ≥Ë°°ÁéØÁîµÊµÅËæìÂá∫ÈôêÂπÖ */
	VAL_LIMIT(balance_pid.balance_out[WHEEL_L], -5000.0f, 5000.0f);
	VAL_LIMIT(balance_pid.balance_out[WHEEL_R], -5000.0f, 5000.0f);
	return 0;
}
